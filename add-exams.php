            ...
            <div class="card-box">
               ...
               <form id="addExams" action="employee/process/processExams.php" method="post" novalidate="novalidate">
                  ...
                  <div class="form-group row">
                     ...
                     <div class="col-md-10">
                        <select id="exam_type_id" class="form-control" name="exam_type_id" onchange="getExamTerm(this.value);">
                           ...
                           <option <?php if (isset($result['fetch_data'][0]['exam_type_id'])) { if ($result['fetch_data'][0]['exam_type_id']==$examTypeList[$i]['id']) { echo 'selected'; } } ?> value="<?php echo $examTypeList[$i][ 'id']; ?>"><?php echo $examTypeList[$i][ 'exam_type']; ?></option>
                           <?php endfor; ?>    
                        </select>
                     </div>
                  </div>
                  ...
                  <div class="form-group row">
                     ...
                     <div class="col-md-10">
                        <select class="form-control" name="section_id" id="section_id" onchange="getExamsSubjectsList(this.value);">
                           <option value='' selected="" disabled="">Select Section</option>
                        </select>
                     </div>
                  </div>
                  
                <div id="main-exams-div" class="form-group row">
                </div>
...
<script type="text/javascript">
   ...
   <?php  if($examId!=''){ ?>        
       section_id='<?php echo $result['fetch_data'][0]['section_id']; ?>';
       getSections('<?php echo $result['fetch_data'][0]['class_id']; ?>');
   
       exam_term_id='<?php echo $result['fetch_data'][0]['exam_term_id']; ?>';
       getExamTerm('<?php echo $result['fetch_data'][0]['exam_type_id']; ?>');
       
       getExamsSubjectsList('<?php echo $result['fetch_data'][0]['section_id']; ?>');
    <?php }?>
   ...
   function getExamsSubjectsList(sectionID){
       $.ajax({
           type: "POST",
           url: "employee/process/processExamType.php",
           data:{type:'getExamsSubjects',sectionID:sectionID},
           beforeSend : function () {
               //$('#wait').html("Wait for checking");
           },
           success:function(data){                
               ...
               if(data.length > 0){
                    for(var i=0;i<data.length;i++){        
                      var htmlDiv="";
                      htmlDiv+='<label class="col-form-label col-md-2">Subject Name</label><div class="col-md-3"><input readonly="readonly" type="text" name="exam_name['+data[i].subId+']" value="'+data[i].subject_name+'" class="exam_name form-control"></div>';

                      htmlDiv+='<label class="col-form-label">Date</label><div class="col-md-3"><input type="text" name="date_of_exam['+data[i].subId+']" class="form-control datetimepicker"></div>';

                      htmlDiv+='<label class="col-form-label">Time</label><div class="col-md-3"><input type="text" name="time_of_exam['+data[i].subId+']" class="form-control timepicker"></div>';

                       $("#main-exams-div").append(htmlDiv);
                   }               
               }else{
                   $("#main-exams-div").html("No result Found!!");
               }
           }
       });
   }
