...
class TransferCertificate extends MySQLCN {
    ...
    function AddTransferCertificate($data) {
        $qry = 'INSERT INTO `transfer_certificate` 
            (`student_name`,`guardian_name`,`address`, `religion`, `joining`,`from_date`,`to_date`,`class_name`,`tc_date`) 
            VALUES ( "'. $data['student_name'] . '", "'. $data['guardian_name'] . '", "'. $data['address'] .'" , "'. $data['religion'] .'" ,"'. $data['joining'].'" ,"'. $data['from_date'].'" ,"'. $data['to_date'].'" ,"'. $data['class_name'].'" ,"'. $data['tc_date'].'")';
        $res = $this->insert($qry);
        if ($res) {
            return true;
        } else {
