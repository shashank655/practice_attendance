...
$resultLoginRecordsList=$teacher->getLoginRecordsLists($teacher_id, $current_year, $current_month);
?>
...
<div class="page-wrapper"> <!-- content -->
			...
			<div class="content-page">
                <form id="searchAttendance" action="teacher-login-records-list.php" method="get" novalidate="novalidate">
                 <div class="row">
                    <?php if($user_role == '1') { ?>
                            ...
                            <table class="table table-striped custom-table">
                                <thead>
                                    <tr>
                                        ...
                                        <th style="min-width:50px;">Logout Time</th>
                                    </tr>
                                </thead>
                                <tbody>
                                <?php $i=1; 
                                    if(!empty($resultLoginRecordsList)) {
                                ?>
                                	<?php foreach ($resultLoginRecordsList as $key => $value) { ?>
                                    <tr>
                                        <td><?php echo $i; ?></td>
                                        <td><?php if(isset($value['login_time'])) 
                                        $showLoginTime = strtotime($value['login_time']);
                                        echo date("F j, Y, g:i a", $showLoginTime);?></td>
                                        <td><?php if(isset($value['logout_time'])) $showLogoutTime = strtotime($value['logout_time']);
                                        echo date("F j, Y, g:i a", $showLogoutTime);?></td>
                                    </tr>
                                	<?php $i++; } } ?>
                                </tbody>
                            </table>
